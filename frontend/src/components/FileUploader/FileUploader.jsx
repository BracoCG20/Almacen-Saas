...
const FileUploader = ({
	...
	// --- MAGIA: Forzar descarga para archivos que vienen del backend (Cross-Origin) ---
	const handleDownloadExisting = async (e) => {
		e.preventDefault(); // Evitamos que abra en nueva pestaña
		try {
			// Obtenemos el archivo en segundo plano
			const response = await fetch(existingUrl);
			const blob = await response.blob();

			// Creamos un link temporal en el navegador y lo "clickeamos"
			...
			// Fallback: si por alguna razón falla, lo abrimos en pestaña normal
			...
			<div className='file-upload-wrapper'>
				<div className='file-preview-card'>
					<div className='file-info'>
						<FileText size={18} className='icon-new' />
	...
	if (existingUrl && !isExistingRemoved) {
		return (
			<div className='file-upload-wrapper'>
				<div className='file-preview-card'>
					<div className='file-info'>
						<FileText size={18} className='icon-existing' />
