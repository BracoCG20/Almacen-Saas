...
const createServicio = async (data, usuarioId) => {
	...
	try {
		...
		// ELIMINADAS: licencias_totales, licencias_usadas
		const query = `
      INSERT INTO servicios (
        ...
        usuario_id_responsable, usuario_creacion_id
...
const updateServicio = async (id, data, usuarioId) => {
	...
	try {
      ...
      UPDATE servicios SET 
        ...
        usuario_id_responsable = $16,
        fecha_modificacion = NOW(), usuario_modificacion_id = $17
...
const anularPago = async (pagoId, usuarioId) => {
	...
	try {
		...
		await client.query(
			"UPDATE historial_pagos SET estado_pago = 'Anulado', fecha_modificacion = NOW(), usuario_modificacion_id = $1 WHERE id = $2",
			[usuarioId, pagoId],
		);
