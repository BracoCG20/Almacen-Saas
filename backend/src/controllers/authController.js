const { pool } = require("../config/db"); // Asegúrate de importar pool correctamente desde tu db.js
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");

// --- 1. LOGIN ---
const login = async (req, res) => {
	// NUEVO: Ahora el campo en BD se llama email_login
	const { email, password } = req.body;

	try {
		// NUEVO: Hacemos JOIN con colaboradores y roles para tener la info completa del usuario
		const query = `
		...
		// NUEVO: Verificar si está activo (ahora se llama 'estado' en la BD)
		if (!user.estado) {
		...
		// NUEVO: Actualizar la fecha de último login
		await pool.query(
		...
		res.json({
			...
			user: {
				id: user.id,
				nombre: `${user.nombres} ${user.apellidos}`, // Concatenamos para el frontend
...
const getPerfil = async (req, res) => {
	try {
		...
		// ¡CORRECCIÓN!: Agregamos u.rol_id al SELECT
		const query = `
...
const updatePerfil = async (req, res) => {
	const id = req.user.id;
	// ¡NUEVO!: Ahora el backend puede recibir todos los datos si el que guarda es un SuperAdmin
	...
	try {
		...
		if (email_login) {
			// Si el SuperAdmin cambia su correo
		...
		// 2. Actualizar tabla COLABORADORES
		const colabFields = [];
		const colabValues = [];
		let colabIdx = 1;
...
// --- 4. REGISTRO (CREAR ACCESO DE USUARIO) ---
const register = async (req, res) => {
	// Recibimos exactamente lo que envía el nuevo modal
	...
	try {
		...
		// 1. Verificar si el correo o nickname ya existe en usuarios
		...
		if (userExist.rows.length > 0) {
			throw new Error(
				"El correo o nickname ya está registrado para otro usuario",
			);
		}
		...
		// 3. Crear el Usuario vinculado al Colaborador
		const usuQuery = `
...
const getAllUsers = async (req, res) => {
	try {
		// NUEVO: Unimos usuarios con colaboradores y empresas
		const query = `
            SELECT 
...
const toggleUserStatus = async (req, res) => {
	const { id } = req.params;
	const { activo } = req.body; // frontend envía true o false
	const modificador_id = req.user.id;

	try {
		// NUEVO: La columna ahora se llama 'estado'
		await pool.query(
...
const adminUpdatePassword = async (req, res) => {
	...
	try {
		...
		// NUEVO: Si el admin le cambia la contraseña, obligamos al usuario a cambiarla la próxima vez que entre
		await pool.query(
