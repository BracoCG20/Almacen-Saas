...
const createProveedor = async (req, res) => {
    ...
    `;
		...
		await client.query(
			`INSERT INTO historial_proveedores (proveedor_id, accion_realizada, descripcion_cambio, usuario_accion_id) 
       VALUES ($1, 'REGISTRO INICIAL', 'Proveedor creado en el sistema.', $2)`,
			[provId, usuarioId],
		);
		...
		res
			.status(400)
			.json({
				error: error.message || "Error interno al registrar el proveedor.",
			});
	} finally {
		client.release();
	}
...
const updateProveedor = async (req, res) => {
	...
	try {
		...
		if (eliminar_contrato === "true") contratoUrl = null;
		if (req.file) contratoUrl = `/uploads/${req.file.filename}`;
    ...
    `;
		...
		await client.query(
			`INSERT INTO historial_proveedores (proveedor_id, accion_realizada, descripcion_cambio, usuario_accion_id) 
       VALUES ($1, 'ACTUALIZACIÓN', 'Se modificaron los datos o el contrato del proveedor.', $2)`,
			[id, usuarioId],
		);
...
/**
 * 4. CAMBIAR ESTADO PROVEEDOR (Baja o Alta Lógica)
 */
const toggleEstadoProveedor = async (req, res) => {
	const { id } = req.params;
	const { estado } = req.body; // true = reactivar, false = desactivar
	const usuarioId = req.user ? req.user.id : null;
...
/**
 * 5. OBTENER HISTORIAL DE UN PROVEEDOR
 */
