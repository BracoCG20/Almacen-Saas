...
const registrarDevolucion = async (req, res) => {
  ...
  try {
    ...
    // <-- AÑADIDO: INSERTAR MOTIVO
    const insertMov = `INSERT INTO historial_movimientos (equipo_id, colaborador_id, tipo_movimiento, fecha_movimiento, cargador_incluido, observaciones, estado_equipo_id, usuario_creacion_id, motivo_movimiento) VALUES ($1, $2, 'devolucion', $3, $4, $5, $6, $7, $8)`;
    await client.query(insertMov, [
      ...
      estado_fisico_id,
      adminId,
      motivo || 'Devolución regular', // Por si viene vacío
    ]);
    ...
    await client.query(
      `INSERT INTO historial_equipos (equipo_id, disponible, estado_fisico_id, observaciones_equipo, accion_realizada, descripcion_cambio, usuario_accion_id) VALUES ($1, $2, $3, $4, 'DEVOLUCIÓN', 'Recepción de equipo en estado: ' || $5 || '. Motivo: ' || $6, $7)`,
      [
        equipo_id,
        estaDisponible,
        estado_fisico_id,
...
const registrarDevolucionConCorreo = async (req, res) => {
  ...
  try {
    ...
    // <-- AÑADIDO: INSERTAR MOTIVO
    const insertMov = `INSERT INTO historial_movimientos (equipo_id, colaborador_id, tipo_movimiento, fecha_movimiento, cargador_incluido, observaciones, estado_equipo_id, correo_enviado, usuario_creacion_id, motivo_movimiento) VALUES ($1, $2, 'devolucion', NOW(), $3, $4, $5, false, $6, $7) RETURNING id`;
    const result = await client.query(insertMov, [
      equipo_id,
      empleado_id,
      cargadorBool,
  ...
  try {
    const query = `
            SELECT 
                ...
                m.observaciones, 
                m.motivo_movimiento as motivo, -- <-- AÑADIDO AL SELECT
